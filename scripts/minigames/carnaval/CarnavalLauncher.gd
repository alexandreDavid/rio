class_name CarnavalLauncher
extends Node2D

# Borne du Sambódromo : lance le défilé de samba (acte 4).
# Première session validée → objectif "lead_samba" coché.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "act4_carnaval_desfile"
const OBJECTIVE_ID: String = "lead_samba"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.CHARISMA, 2, "carnaval_sessions", 10)
		ReputationSystem.gain_capped(ReputationSystem.Axis.CIVIC, 1, "carnaval_sessions", 10)
