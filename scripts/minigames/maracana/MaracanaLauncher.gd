class_name MaracanaLauncher
extends Node2D

# Borne du stand de la torcida au Maracanã. Lance le minijeu de réactions.
# Tips → argent crédité ; première session réussie → objectif "lead_torcida" coché.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "ronaldo_maracana_torcida"
const OBJECTIVE_ID: String = "lead_torcida"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.STREET, 1, "torcida_sessions", 5)
		ReputationSystem.gain_capped(ReputationSystem.Axis.CIVIC, 1, "torcida_sessions", 5)
