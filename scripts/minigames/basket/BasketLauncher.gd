class_name BasketLauncher
extends Node2D

# Borne du terrain de basket à l'Aterro do Flamengo. Lance le minijeu de tirs
# au panier. Première session gagnée → objectif "win_basket_session" coché.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "ronaldo_aterro_basket"
const OBJECTIVE_ID: String = "win_basket_session"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.STREET, 1, "basket_sessions", 5)
