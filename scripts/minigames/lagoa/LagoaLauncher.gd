class_name LagoaLauncher
extends Node2D

# Borne de location de vélo à la Lagoa. Lance le mini-jeu de circuit.
# Tip total → argent crédité. Première boucle complétée coche l'objectif
# 'complete_lap' de la quête carlos_lagoa_volta.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "carlos_lagoa_volta"
const OBJECTIVE_ID: String = "complete_lap"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.TOURIST, 1, "lagoa_laps", 5)
		ReputationSystem.gain_capped(ReputationSystem.Axis.CIVIC, 1, "lagoa_laps", 5)
