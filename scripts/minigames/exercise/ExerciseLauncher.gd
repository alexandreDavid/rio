...
func _on_match_ended(qualifies: bool, _reps: int) -> void:
	if qualifies:
		# Source "gym" plafonnée à 3 points de Charisma sur toute la partie.
		ReputationSystem.gain_capped(ReputationSystem.Axis.CHARISMA, 1, "gym", 3)
	if _active_match:
