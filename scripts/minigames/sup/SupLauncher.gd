class_name SupLauncher
extends Node2D

# Borne de location de stand-up paddle aux Cagarras. Lance le minijeu de circuit.
# Première session complète → objectif "round_islands" coché.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "pedro_cagarras_sup"
const OBJECTIVE_ID: String = "round_islands"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.TOURIST, 1, "sup_sessions", 5)
