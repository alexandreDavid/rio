class_name ValetLauncher
extends Node2D

# Borne du stand valet du Copa Palace.
# Lance le minijeu de service. Les tips totaux deviennent l'argent crédité au joueur.
# Première réussite (>= 1 client servi) coche l'objectif de la quête valet_palace.

@export var interactable: Interactable
@export var match_scene: PackedScene

const QUEST_ID: String = "valet_palace"
const OBJECTIVE_ID: String = "first_valet_shift"
...
func _on_match_ended(qualifies: bool, tips: int) -> void:
	if tips > 0:
		...
		if inv:
			inv.add_money(tips)
	if qualifies:
		ReputationSystem.gain_capped(ReputationSystem.Axis.TOURIST, 1, "valet_shifts", 5)
